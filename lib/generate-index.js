...
// 新增：歌词和封面刮削API配置
const LYRICS_API = process.env.LYRICS_API || 'https://api.lyrics.ovh/v1'; // 歌词API
const COVER_API = process.env.COVER_API || 'https://api.deezer.com/search'; // 封面API
...
async function processMusicFiles(files, folderPath) {
  ...
  return musicFiles.map(file => {
    ...
    // 提取可能的歌手和歌曲名（假设文件名格式为 "歌手 - 歌曲名" 或 "歌曲名"）
    ...
    if (fileNameParts.length >= 2) {
      ...
      songName = fileNameParts.slice(1).join(' - ');
    }
...
<body>
        ...
        <div class="player-modal-content">
            ...
            <div class="player-view" id="player-view">
                ...
                <div class="player-modal-track-info">
                    ...
                    <!-- 刮削按钮 -->
                    <button class="scrape-btn" id="scrape-lyrics-btn">
                        <i class="fas fa-search"></i>
                        <span>搜索歌词</span>
                    </button>
                    <button class="scrape-btn" id="scrape-cover-btn" style="display: none;">
            ...
            <div class="lyrics-view" id="lyrics-view">
                ...
                <div class="lyrics-container" id="lyrics-container">
                    <div class="no-lyrics">
                        ...
                        <p>加载歌词中...</p>
                        <p style="font-size: 14px; margin-top: 10px;">点击下方按钮搜索歌词</p>
                        <button class="scrape-btn" id="lyrics-scrape-btn">
                            <i class="fas fa-search"></i>
                            <span>搜索歌词</span>
                        </button>
                    </div>
    ...
    <script>
        ...
        let musicData = {
            ...
            // 新增：刮削相关
            isScrapingLyrics: false,
            isScrapingCover: false
        };
        ...
        const lyricsScrapeBtn = document.getElementById('lyrics-scrape-btn');
        const scrapeLyricsBtn = document.getElementById('scrape-lyrics-btn');
        const scrapeCoverBtn = document.getElementById('scrape-cover-btn');
        ...
        function initEventListeners() {
            ...
            // 模态框封面点击切换到歌词视图
            playerModalCover.addEventListener('click', showLyricsView);
            
            // 模态框关闭
            playerModalClose.addEventListener('click', closePlayerModal);
            
            // 歌词视图关闭
            lyricsViewClose.addEventListener('click', showPlayerView);
            
            // 刮削按钮
            scrapeLyricsBtn.addEventListener('click', scrapeLyrics);
            scrapeCoverBtn.addEventListener('click', scrapeCover);
            lyricsScrapeBtn.addEventListener('click', scrapeLyrics);
        ...
        function loadTrack(albumIndex, trackIndex, playImmediately = true) {
            ...
            // 显示或隐藏刮削封面按钮
            if (!album.cover) {
                scrapeCoverBtn.style.display = 'flex';
            } else {
                scrapeCoverBtn.style.display = 'none';
            }
        ...
        function updateCovers(album) {
            ...
            if (currentTrack && currentTrack.scrapedCover) {
                coverUrl = currentTrack.scrapedCover;
            }
            
            // 播放器封面
            const coverHtml = coverUrl 
                ? \`<img src="\${coverUrl}" alt="\${album.name}" onerror="this.style.display='none'; this.parentNode.innerHTML='<i class=\\"fas fa-music\\"></i>';">\`
                : '<i class="fas fa-music"></i>';
            
            playerCover.innerHTML = coverHtml;
            playerModalCover.innerHTML = coverHtml + '<div class="cover-hint">点击查看歌词</div>';
        ...
        async function loadLyrics(lrcUrl) {
            try {
                ...
                // 检查是否有刮削的歌词
                const currentTrack = musicData.currentAlbumTracks[musicData.currentTrackIndex];
                if (currentTrack && currentTrack.scrapedLyrics) {
                    musicData.lyrics = parseLRC(currentTrack.scrapedLyrics);
                    renderLyrics();
                } else {
                    renderNoLyrics();
                }
        ...
        async function scrapeLyrics() {
            if (musicData.isScrapingLyrics) return;
            
            const currentTrack = musicData.currentAlbumTracks[musicData.currentTrackIndex];
            if (!currentTrack) return;
            
            musicData.isScrapingLyrics = true;
            scrapeLyricsBtn.classList.add('scraping');
            lyricsScrapeBtn.classList.add('scraping');
            
            // 尝试从歌词API获取
            try {
                let artist = currentTrack.artist || '';
                let songName = currentTrack.songName || currentTrack.displayName;
                ...
                console.log('搜索歌词:', artist, '-', songName);
                
                // 使用歌词API
                const response = await fetch(\`https://api.lyrics.ovh/v1/\${encodeURIComponent(artist)}/\${encodeURIComponent(songName)}\`);
                
                if (response.ok) {
                    const data = await response.json();
                    if (data.lyrics) {
                        // 保存刮削的歌词到当前曲目
                        currentTrack.scrapedLyrics = data.lyrics;
                        musicData.lyrics = parseLRC(data.lyrics);
                        renderLyrics();
                        
                        // 更新刮削按钮状态
                        scrapeLyricsBtn.innerHTML = '<i class="fas fa-check"></i><span>歌词已找到</span>';
                        lyricsScrapeBtn.innerHTML = '<i class="fas fa-check"></i><span>歌词已找到</span>';
                ...
                console.log('刮削歌词失败:', error);
                
                // 尝试备选API
                try {
                    const response = await fetch(\`https://some-other-lyrics-api.com/search?q=\${encodeURIComponent(currentTrack.displayName)}\`);
                    if (response.ok) {
                        const data = await response.json();
                        if (data.lyrics) {
                            currentTrack.scrapedLyrics = data.lyrics;
                    ...
                    console.log('备选歌词API也失败:', e2);
                }
                
                // 显示错误
                renderNoLyrics('未找到歌词，请尝试手动添加 .lrc 文件');
                scrapeLyricsBtn.innerHTML = '<i class="fas fa-exclamation-triangle"></i><span>搜索失败</span>';
                lyricsScrapeBtn.innerHTML = '<i class="fas fa-exclamation-triangle"></i><span>搜索失败</span>';
            } finally {
                musicData.isScrapingLyrics = false;
                setTimeout(() => {
                    scrapeLyricsBtn.classList.remove('scraping');
                    lyricsScrapeBtn.classList.remove('scraping');
                }, 1000);
            }
        }
        ...
        async function scrapeCover() {
            if (musicData.isScrapingCover) return;
            
            const currentTrack = musicData.currentAlbumTracks[musicData.currentTrackIndex];
            const album = musicData.albums[musicData.currentAlbumIndex];
            if (!currentTrack || !album) return;
            
            musicData.isScrapingCover = true;
            scrapeCoverBtn.classList.add('scraping');
            ...
            try {
                ...
                console.log('搜索封面:', artist, '-', songName);
                
                // 使用Deezer API搜索
                const response = await fetch(\`https://api.deezer.com/search?q=\${encodeURIComponent(artist + ' ' + songName)}&limit=1\`);
                
                if (response.ok) {
                    const data = await response.json();
                    if (data.data && data.data.length > 0 && data.data[0].album && data.data[0].album.cover_xl) {
                        const coverUrl = data.data[0].album.cover_xl;
                        
                        // 保存刮削的封面到当前曲目
                        currentTrack.scrapedCover = coverUrl;
                        
                        // 更新封面显示
                        updateCovers(album);
                        
                        // 更新刮削按钮状态
                        scrapeCoverBtn.innerHTML = '<i class="fas fa-check"></i><span>封面已找到</span>';
                ...
                // 尝试搜索专辑封面
                const albumResponse = await fetch(\`https://api.deezer.com/search?q=\${encodeURIComponent(album.name)}&limit=1\`);
                if (albumResponse.ok) {
                ...
                console.log('刮削封面失败:', error);
                scrapeCoverBtn.innerHTML = '<i class="fas fa-exclamation-triangle"></i><span>搜索失败</span>';
            } finally {
                musicData.isScrapingCover = false;
                setTimeout(() => {
                    scrapeCoverBtn.classList.remove('scraping');
                }, 1000);
            }
        }
        ...
        function saveScrapedData() {
            try {
                ...
                musicData.albums.forEach((album, albumIndex) => {
                    album.tracks.forEach((track, trackIndex) => {
                        if (track.scrapedLyrics || track.scrapedCover) {
                            if (!scrapedData[albumIndex]) scrapedData[albumIndex] = {};
                            if (!scrapedData[albumIndex][trackIndex]) scrapedData[albumIndex][trackIndex] = {};
                            
                            if (track.scrapedLyrics) {
                                scrapedData[albumIndex][trackIndex].lyrics = track.scrapedLyrics;
                            }
                            if (track.scrapedCover) {
                                scrapedData[albumIndex][trackIndex].cover = track.scrapedCover;
                            }
                        }
                    });
                });
        ...
        function loadScrapedData() {
            try {
                ...
                Object.keys(scrapedData).forEach(albumIndex => {
                    Object.keys(scrapedData[albumIndex]).forEach(trackIndex => {
                        const ai = parseInt(albumIndex);
                        const ti = parseInt(trackIndex);
                        
                        if (musicData.albums[ai] && musicData.albums[ai].tracks[ti]) {
                            const track = musicData.albums[ai].tracks[ti];
                            const data = scrapedData[albumIndex][trackIndex];
                            
                            if (data.lyrics) track.scrapedLyrics = data.lyrics;
                            if (data.cover) track.scrapedCover = data.cover;
                        }
                    });
                });
            } catch (e) {
                console.error('加载刮削数据失败:', e);
            }
        }
        ...
        function renderNoLyrics(message = '暂无歌词') {
            ...
            // 重新绑定事件
            document.getElementById('lyrics-scrape-btn').addEventListener('click', scrapeLyrics);
        }
